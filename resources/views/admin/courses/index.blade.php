            ...
            @forelse($courses as $course)
                <div class="item-card flex flex-col md:flex-row gap-y-10 justify-between md:items-center">
                    ...
                    <div class="hidden md:flex flex-col">
                        ...
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $course->students ? $course->students->count() : 0 }}</h3>
                    </div>
                    <div class="hidden md:flex flex-col">
                        <p class="text-slate-500 text-sm">Videos</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $course->course_video ? $course->course_video->count() : 0 }}</h3>
                    </div>
                    <div class="hidden md:flex flex-col">
                        <p class="text-slate-500 text-sm">Teacher</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ $course->teacher && $course->teacher->user ? $course->teacher->user->name : 'N/A' }}</h3>
                    </div>
